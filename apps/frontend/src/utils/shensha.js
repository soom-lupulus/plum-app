...
function queryShenSha(ganzhi, bazi, isman, witch, niannayin) {
  ...
  if (
    (witch != 3 && kongwang(rigan + rizhi, zhi) == 1) ||
    (witch != 1 && kongwang(niangan + nianzhi, zhi) == 1)
  ) {
    shengShaList.push('空亡')
  }
  if (
    (witch == 3 && tongzi(yuezhi, niannayin, rizhi)) ||
    (witch == 4 && tongzi(yuezhi, niannayin, shizhi))
  ) {
...
function dexiuguiren(yuezhi, tiangans) {
  ...
  // 水局：申子辰
    ...
    return hasTiangan(['壬', '癸', '戊', '己']) &&
      (hasTianganCombination('丙', '辛') || hasTianganCombination('甲', '己'))
      ? 1
      : 0
  }
...
function tianyiguiren(paramString1, paramString2) {
  ...
  return Array.isArray(conditions[paramString1]) && conditions[paramString1].includes(paramString2)
    ? 1
    : 0
}
...
function ciguan(paramString1, paramString2, paramString3) {
  ...
  if (matchConditions[nywx]) {
    ...
    if (
      paramString3 === mainMatch ||
      (Array.isArray(subMatch) && subMatch[0] === paramString2 && paramString3 === subMatch[1])
    ) {
      return 1
    }
  }
...
function tianganYinyang(tiangan) {
  if (tiangan == '甲' || tiangan == '丙' || tiangan == '戊' || tiangan == '庚' || tiangan == '壬') {
    return true
  }
  if (tiangan == '乙' || tiangan == '丁' || tiangan == '己' || tiangan == '辛' || tiangan == '癸') {
    return false
  }
  return false
}
